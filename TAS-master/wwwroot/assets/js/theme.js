...
function layout_change_default() {
    ...
    e && e.classList.add("active"), window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change", e => {
        layout_change(t = e.matches ? "dark" : "light")
    })
}
document.addEventListener("DOMContentLoaded", function () {
    var e;
    "undefined" != typeof Storage ? (e = localStorage.getItem("theme")) && layout_change(e) : console.warn("Web Storage API is not supported in this browser.")
});
for (var layout_btn = document.querySelectorAll(".theme-layout .btn"), t = 0; t < layout_btn.length; t++) layout_btn[t] && layout_btn[t].addEventListener("click", function (e) {
    e.stopPropagation();
    e = e.target;
...
function layout_theme_contrast_change(e) {
    var t = document.getElementsByTagName("body")[0],
        ...
        t = (t.setAttribute("data-pc-theme_contrast", e), a && a.classList.remove("active"), document.querySelector(`.theme-contrast .btn[data-value='${e}']`));
    t && t.classList.add("active")
}

function layout_caption_change(e) {
    "true" == e ? (document.getElementsByTagName("body")[0].setAttribute("data-pc-sidebar-caption", "true"), document.querySelector(".theme-nav-caption .btn.active") && (document.querySelector(".theme-nav-caption .btn.active").classList.remove("active"), document.querySelector(".theme-nav-caption .btn[data-value='true']").classList.add("active"))) : (document.getElementsByTagName("body")[0].setAttribute("data-pc-sidebar-caption", "false"), document.querySelector(".theme-nav-caption .btn.active") && (document.querySelector(".theme-nav-caption .btn.active").classList.remove("active"), document.querySelector(".theme-nav-caption .btn[data-value='false']").classList.add("active")))
}

function preset_change(e) {
    document.getElementsByTagName("body")[0].setAttribute("data-pc-preset", e), document.querySelector(".pct-offcanvas") && (document.querySelector(".preset-color > a.active").classList.remove("active"), document.querySelector(".preset-color > a[data-value='" + e + "']").classList.add("active"))
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    a && a.addEventListener("click", function (e) {
        localStorage.clear(), location.reload(), localStorage.setItem("layout", "vertical")
    })
});